...
function createMiddleware(options = {}) {
  ...
  // Database connection
  const mongoUri = options.mongodbUri || options.dbConnection || process.env.MONGODB_URI;
  
  if (!mongoUri && mongoose.connection.readyState !== 1) {
    console.warn('⚠️  Warning: No MongoDB connection provided to middleware. Set MONGODB_URI in environment or pass mongodbUri/dbConnection option.');
  } else if (mongoUri && mongoose.connection.readyState !== 1) {
    mongoose.connect(mongoUri)
      .then(() => console.log('✅ Middleware: Connected to MongoDB'))
      .catch(err => console.error('❌ Middleware: MongoDB connection error:', err));
  ...
  router.use('/admin/assets', express.static(path.join(__dirname, '..', 'public')));
  ...
  router.get('/admin/global-settings', basicAuth, (req, res) => {
    const templatePath = path.join(__dirname, '..', 'views', 'admin-global-settings.ejs');
    fs.readFile(templatePath, 'utf8', (err, template) => {
      if (err) {
        console.error('Error reading template:', err);
        return res.status(500).send('Error loading page');
