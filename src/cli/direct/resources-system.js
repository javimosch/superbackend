  ...
  async execute(options) {
    ...
    switch (options.command) {
      ...
      case 'create': {
        ...
        if (!options.description) throw new Error('--description (cron expression) is required');
        const cron = await CronJob.create({ name: options.name, description: options.description, enabled: true });
        return cron;
      }
      case 'delete': {
        if (!options.id) throw new Error('Cron ID is required');
      ...
      case 'enable': {
        if (!options.id) throw new Error('Cron ID is required');
        const cron = await CronJob.findByIdAndUpdate(options.id, { enabled: true }, { new: true });
        if (!cron) throw new Error('Cron not found');
        return cron;
      }
      case 'disable': {
        if (!options.id) throw new Error('Cron ID is required');
        const cron = await CronJob.findByIdAndUpdate(options.id, { enabled: false }, { new: true });
        if (!cron) throw new Error('Cron not found');
        return cron;
      }
      default:
...
const errors = {
  async execute(options) {
    const ErrorEntry = mongoose.model('ErrorEntry');
    switch (options.command) {
      case 'list': {
        const limit = parseInt(options.value) || 50;
        const errors = await ErrorEntry.find().sort({ createdAt: -1 }).limit(limit).lean();
        return { items: errors, count: errors.length };
      }
      case 'get': {
        if (!options.id) throw new Error('Error ID is required');
        const error = await ErrorEntry.findById(options.id).lean();
        if (!error) throw new Error('Error not found');
        return error;
      }
      case 'delete': {
        if (!options.id) throw new Error('Error ID is required');
        const error = await ErrorEntry.findByIdAndDelete(options.id);
        if (!error) throw new Error('Error not found');
        return { success: true, id: options.id };
      }
      case 'clear': {
        await ErrorEntry.deleteMany({});
...
const scripts = {
  async execute(options) {
    ...
    switch (options.command) {
      ...
      case 'create': {
        if (!options.name) throw new Error('--name is required');
        if (!options.description) throw new Error('--description (script code) is required');
        const script = await ScriptDefinition.create({ name: options.name, description: options.description, enabled: true });
        return script;
      }
      case 'delete': {
        if (!options.id) throw new Error('Script ID is required');
...
const workflows = {
  async execute(options) {
    ...
    switch (options.command) {
      ...
      case 'create': {
        if (!options.name) throw new Error('--name is required');
        const workflow = await Workflow.create({ name: options.name, description: options.description || '', enabled: true });
        return workflow;
      }
      case 'delete': {
        if (!options.id) throw new Error('Workflow ID is required');
      ...
      case 'enable': {
        if (!options.id) throw new Error('Workflow ID is required');
        const workflow = await Workflow.findByIdAndUpdate(options.id, { enabled: true }, { new: true });
        if (!workflow) throw new Error('Workflow not found');
        return workflow;
      }
      case 'disable': {
        if (!options.id) throw new Error('Workflow ID is required');
        const workflow = await Workflow.findByIdAndUpdate(options.id, { enabled: false }, { new: true });
        if (!workflow) throw new Error('Workflow not found');
        return workflow;
      }
      default:
...
const healthChecks = {
  async execute(options) {
    ...
    switch (options.command) {
      ...
      case 'create': {
        if (!options.name) throw new Error('--name is required');
        const check = await HealthCheck.create({ name: options.name, description: options.description || '', enabled: true });
        return check;
      }
      case 'delete': {
        if (!options.id) throw new Error('Health check ID is required');
