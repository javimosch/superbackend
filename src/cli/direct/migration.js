...
const addTimestamps = {
  async execute(options, context) {
    ...
    if (dryRun) {
      return { collection: collectionName, dryRun: true, docsToUpdate: docs.length, samples: docs.slice(0, 5).map(d => ({ _id: d._id })) };
    }
    ...
    for (const doc of docs) {
      await collection.updateOne({ _id: doc._id }, { $set: { createdAt: doc.createdAt || now, updatedAt: doc.updatedAt || now } });
      updatedCount++;
    }
...
const dataDigest = {
  async execute(options, context) {
    ...
    for (const coll of collections) {
      ...
      digest.collections.push({ name: coll.name, count: stats.count, size: stats.size, storageSize: stats.storageSize });
    }
