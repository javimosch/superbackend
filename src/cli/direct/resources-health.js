...
const healthIncidents = {
  async execute(options) {
    ...
    switch (options.command) {
      case 'list': {
        const incidents = await HealthIncident.find().sort({ createdAt: -1 }).lean();
...
const healthAttempts = {
  async execute(options) {
    ...
    switch (options.command) {
      case 'list': {
        const attempts = await HealthAutoHealAttempt.find().sort({ createdAt: -1 }).lean();
...
const errorAggregates = {
  async execute(options) {
    ...
    switch (options.command) {
      case 'list': {
        const aggregates = await ErrorAggregate.find().sort({ count: -1 }).lean();
...
const metricBuckets = {
  async execute(options) {
    const MetricBucket = mongoose.model('MetricBucket');
    switch (options.command) {
      case 'list': {
        const limit = parseInt(options.value) || 50;
        const buckets = await MetricBucket.find().sort({ createdAt: -1 }).limit(limit).lean();
        return { items: buckets, count: buckets.length };
      }
      case 'clear': {
        const result = await MetricBucket.deleteMany({});
        return { success: true, deletedCount: result.deletedCount };
      }
      default:
...
const virtualEjsFiles = {
  async execute(options) {
    ...
    switch (options.command) {
      ...
      default:
        throw new Error(`Unknown virtual-ejs-files command: ${options.command}`);
    }
  },
};
...
const virtualEjsGroups = {
  async execute(options) {
    ...
    switch (options.command) {
      ...
      default:
        throw new Error(`Unknown virtual-ejs-groups command: ${options.command}`);
    }
  },
};

module.exports = { healthIncidents, healthAttempts, errorAggregates, metricBuckets, virtualEjsFiles, virtualEjsGroups };
