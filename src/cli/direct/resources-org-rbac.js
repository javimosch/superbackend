  ...
  async execute(options) {
    ...
    switch (options.command) {
      ...
      case 'get': {
        ...
        const org = await Org.findById(options.id).lean();
        if (!org) throw new Error('Org not found');
        return org;
      }
      case 'create': {
        if (!options.name) throw new Error('--name is required');
        const org = await Org.create({ name: options.name, description: options.description || '' });
        return org;
      }
      case 'update': {
        if (!options.id) throw new Error('Org ID is required');
        ...
        const org = await Org.findByIdAndUpdate(options.id, updateData, { new: true });
        if (!org) throw new Error('Org not found');
        return org;
      }
      case 'delete': {
...
const rbacRoles = {
  async execute(options) {
    ...
    switch (options.command) {
      ...
      case 'create': {
        if (!options.name) throw new Error('--name is required');
        const role = await RbacRole.create({ name: options.name, description: options.description || '' });
        return role;
      }
      case 'delete': {
        if (!options.id) throw new Error('Role ID is required');
...
const rbacGroups = {
  async execute(options) {
    ...
    switch (options.command) {
      ...
      case 'create': {
        if (!options.name) throw new Error('--name is required');
        const group = await RbacGroup.create({ name: options.name, description: options.description || '' });
        return group;
      }
      case 'delete': {
        if (!options.id) throw new Error('Group ID is required');
  ...
  async execute(options) {
    ...
    switch (options.command) {
      case 'list': {
        ...
        const invites = await Invite.find().sort({ createdAt: -1 }).limit(limit).lean();
...
const orgMembers = {
  async execute(options) {
    ...
    switch (options.command) {
      case 'list': {
        const members = await OrganizationMember.find().populate('userId').lean();
