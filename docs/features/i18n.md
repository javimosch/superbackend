...
If you mount SuperBackend under a different prefix (example `/superbackend`), use:
- `/superbackend/api/i18n/bundle`

The admin UI is also prefixed:
- `/saas/admin/i18n`
- `/superbackend/admin/i18n`

## Required app configuration

### 1) Mount SuperBackend middleware
SuperBackend must be mounted so the i18n API/admin routes exist.
...
- EJS pages will *not* have `t()` and you won’t be able to render translations server-side.
...
## i18n AI provider/model defaults
The i18n admin UI includes AI translation helpers.

Defaults resolve in this order:
1. Request body: `providerKey` / `model`
2. Centralized defaults:
   - System defaults:
     - `llm.systemDefaults.i18n.translate.preview.{providerKey,model}`
     - `llm.systemDefaults.i18n.translate.text.{providerKey,model}`
   - Global defaults: `llm.defaults.{providerKey,model}`
3. Legacy fallback model:
   - `i18n.ai.model`
4. Environment fallback (last resort):
   - `DEFAULT_LLM_PROVIDER_KEY`
   - `DEFAULT_LLM_MODEL`

Legacy OpenRouter API key overrides (used when the resolved provider is `openrouter`):
...
The host app owns this script (SuperBackend only provides the bundle endpoint).
