...
// Database connection
mongoose.connect(process.env.MONGODB_URI || 'mongodb://localhost:27017/notesyncer')
...
const webhookHandler = require('./src/controllers/billing.controller').handleWebhook;
...
app.get('/', (req, res) => {
  res.render('index', {
    ...
    description: 'Automate Your Substack Notes. Publish Smarter, Grow Faster. AI generates, schedules, and optimizes notes in minutes—reclaim 200+ hours/year.'
  });
});
...
app.listen(PORT, () => {
  ...
  console.log('  POST /api/billing/create-checkout-session - Create Stripe checkout');
  console.log('  POST /api/billing/create-portal-session - Create billing portal');
  console.log('  POST /api/billing/reconcile-subscription - Reconcile subscription');
  ...
  console.log('  PUT  /api/admin/users/:id/subscription - Update subscription (Basic Auth)');
  console.log('  POST /api/admin/users/:id/reconcile - Reconcile user (Basic Auth)');
  console.log('  POST /api/admin/generate-token - Generate JWT for testing (Basic Auth)');
  console.log('  GET  /api/admin/stripe-webhooks - List webhook events (Basic Auth)');
  console.log('  GET  /api/admin/stripe-webhooks/:id - Get webhook event (Basic Auth)');
  console.log('  GET  /api/notifications - Get user notifications (JWT)');
  console.log('  PUT  /api/notifications/:id/read - Mark notification as read (JWT)');
  ...
  console.log('  POST /api/user/password-reset-request - Request password reset');
  console.log('  POST /api/user/password-reset-confirm - Confirm password reset');
  ...
  console.log('  GET  /api/admin/settings - Get all global settings (Basic Auth)');
  console.log('  GET  /api/admin/settings/:key - Get specific setting (Basic Auth)');
  console.log('  PUT  /api/admin/settings/:key - Update setting (Basic Auth)');
  console.log('  POST /api/admin/settings - Create new setting (Basic Auth)');
  console.log('  DELETE /api/admin/settings/:key - Delete setting (Basic Auth)');
  console.log('  GET  /api/admin/settings/public - Get public settings (No Auth)');
  console.log('');
  console.log('🔧 Admin Testing UI:');
  console.log('  GET  /admin/test - API Testing Interface (Basic Auth)');
  console.log('  GET  /admin/global-settings - Global Settings Manager (Basic Auth)');
});